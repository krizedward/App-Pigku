      ...
      <div class="card-body">
        <div class="container">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <input type="hidden" name="expenses[{{ $index }}][description_expense]" value="{{ $expense->description_expense }}">
                    </td>
                    <td>
                      {{ $expense->m_kategori->name_kategori ?? '-' }}
                      <input type="hidden" name="expenses[{{ $index }}][kategori_id]" value="{{ $expense->kategori_id }}">
                    </td>
                    <td>
                      Rp{{ number_format($expense->total_price, 0, ',', '.') }}
                      <input type="hidden" name="expenses[{{ $index }}][total_price]" value="{{ $expense->total_price }}">
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <input type="hidden" name="expenses[{{ $index }}][note_expense]" value="{{ $expense->note_expense }}">
                    </td>
                    <td>
                      <button type="submit" form="delete-form-{{ $expense->id }}" 
          ...
          <form id="delete-form-{{ $expense->id }}" 
                action="{{ route('tempexpense.destroy', $expense->id) }}" 
...
<script>
  ...
  document.getElementById('expenseForm').addEventListener('submit', function (e) {
    let dateInput = document.getElementById('date_expense').value;
    sessionStorage.setItem('date_expense', dateInput);
  });

  // Saat halaman dimuat
  window.addEventListener('DOMContentLoaded', function () {
    let savedDate = sessionStorage.getItem('date_expense');

    if (savedDate) {
      // kalau ada simpanan, pakai yang tersimpan
      document.getElementById('date_expense').value = savedDate;
    } else {
      // kalau tidak ada simpanan, pakai tanggal hari ini
      let today = new Date().toISOString().split('T')[0];
      document.getElementById('date_expense').value = today;
    }
  });
</script>
@endsection